...
After clone on the VPS you **create** `.env` from the template, run `graphify .` once, and Sunny/Maya create `.sunny/` at runtime.
...
# 3) Environment (never commit .env)
cp .env.example .env
...
# Set: DOMAIN, ACME_EMAIL, POSTGRES_PASSWORD, JWT secret (openssl rand -base64 64)
